...
@pytest.fixture()
def small_dataset(labeled_file_df, overlay_pre):
    return AudioFileDataset(labeled_file_df, overlay_pre)
...
def test_audio_file_dataset(labeled_file_df, pre):
...
def test_audio_file_dataset_no_reshape(labeled_file_df, pre):
    ...
    dataset = AudioFileDataset(labeled_file_df, pre)
    sample1 = dataset[0]
    # should be the same shape as loading audio and creating spec
    from opensoundscape import Audio, Spectrogram

    spec_shape = (
        Spectrogram.from_audio(
            Audio.from_file(
                "tests/audio/silence_10s.mp3", duration=2, sample_rate=pre.sample_rate
            )
        ).bandpass(0, 11025)
    ).spectrogram.shape
    assert sample1.data.numpy().shape == (1, 257, 171)  # (1, 129, 343)
    assert sample1.data.numpy().shape == tuple([1] + list(spec_shape))


def test_spec_preprocessor_augment_off(labeled_file_df, pre):
    """should return same image each time"""
    dataset = AudioFileDataset(labeled_file_df, pre, bypass_augmentations=True)
    sample1 = dataset[0].data.numpy()
    sample2 = dataset[0].data.numpy()
    assert np.array_equal(sample1, sample2)


def test_spec_preprocessor_augment_on(labeled_file_df, pre):
...
def test_spec_preprocessor_overlay(labeled_file_df, overlay_pre):
    dataset = AudioFileDataset(labeled_file_df, overlay_pre)
    sample1 = dataset[0].data
...
def test_overlay_tries_different_sample(labeled_file_df, bad_good_df):
...
def test_overlay_different_class(labeled_file_df, overlay_pre):
...
def test_overlay_no_valid_samples(labeled_file_df, overlay_df_all_positive):
    pre = SpectrogramPreprocessor(
        sample_duration=2.0,
        sample_rate=22050,
...
def test_overlay_with_weight_range(labeled_file_df, overlay_pre):
...
def test_overlay_with_invalid_weight_range(labeled_file_df, overlay_pre):
    """overlay should allow range [min,max] for overlay_weight"""
    dataset = AudioFileDataset(labeled_file_df, overlay_pre)
...
def test_overlay_update_labels(labeled_file_df, overlay_pre):
    """should return different images each time"""
    dataset = AudioFileDataset(labeled_file_df, overlay_pre)
...
def test_overlay_update_labels_duplicated_index(labeled_file_df, overlay_df):
...
def test_overlay_criterion_fn(labeled_clip_df, overlay_pre):
    """should only return a different overlay if
    criterion_fn returns True
    """
    dataset = AudioFileDataset(labeled_clip_df, overlay_pre)
    # dataset.preprocessor.pipeline.overlay.set(overlay_class="different")
    dataset.preprocessor.pipeline.overlay.set(criterion_fn=lambda x: x.labels[0] == 1)
...
def test_audio_splitting_dataset(labeled_file_df, pre):
    dataset = AudioFileDataset(labeled_file_df, pre)
...
def test_audio_splitting_dataset_overlap(labeled_file_df, pre):
    dataset = AudioFileDataset(
        labeled_file_df, pre, overlap_fraction=0.5, final_clip=None
    )
...
def test_audio_splitting_dataset_overlap_rounding(labeled_file_df):
    ...
    dataset = AudioFileDataset(
        labeled_file_df, audio_pre, overlap_fraction=0.1, final_clip=None
    )
    for x in dataset:
        assert len(x.data.samples) == 48000 * 2


def test_get_item_with_list_index_raises_error(labeled_file_df, pre):
    dataset = AudioFileDataset(labeled_file_df, pre)
