...
def raven_annotations_true_annots(request, raven_annots_dir):
    ...
    path = Path(
        f"{raven_annots_dir}/MSD-0003_20180427_2minstart00-15.Table.1.selections.txt.lower"
    )
...
def test_raven_split_single_annotation_empty(
    raven_annotations_empty,
):
    result_df = raven.split_single_annotation(
...
def test_raven_split_starts_ends_empty(
    raven_annotations_empty,
):
