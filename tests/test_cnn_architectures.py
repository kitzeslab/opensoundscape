...
def test_freeze_feature_extractor():
    """should disable grad on feature extractor but not classifier"""
    arch = cnn_architectures.resnet18(2, freeze_feature_extractor=True, weights=None)
...
def test_modify_out_shape():
    ...
    arch = cnn_architectures.resnet18(10, weights=None)
    assert arch.fc.out_features == 10


def test_freeze_params():
    """tests that model parameters are frozen"""
    arch = cnn_architectures.resnet18(100, weights=None)
...
def test_resnet18():
    arch = cnn_architectures.resnet18(num_classes=2, weights=None)
    assert isinstance(arch, torch.nn.Module)


def test_resnet34():
    arch = cnn_architectures.resnet34(10, weights=None)
    assert isinstance(arch, torch.nn.Module)
...
def test_resnet101():
    arch = cnn_architectures.resnet101(4, weights=None)
    assert isinstance(arch, torch.nn.Module)


def test_resnet152():
    arch = cnn_architectures.resnet152(3, weights=None)
    assert isinstance(arch, torch.nn.Module)


def test_alexnet():
    arch = cnn_architectures.alexnet(2, weights=None)
    assert isinstance(arch, torch.nn.Module)


def test_vgg11_bn():
    arch = cnn_architectures.vgg11_bn(2, weights=None)
    assert isinstance(arch, torch.nn.Module)


def test_squeezenet1_0():
    arch = cnn_architectures.squeezenet1_0(10, weights=None)
    assert isinstance(arch, torch.nn.Module)


def test_densenet121():
    arch = cnn_architectures.densenet121(111, weights=None)
    assert isinstance(arch, torch.nn.Module)
...
def test_load_default_weights():
    arch = cnn_architectures.resnet18(4, weights="DEFAULT")
    assert isinstance(arch, torch.nn.Module)
...
def test_efficientnet_b4():
    arch = cnn_architectures.efficientnet_b4(2, weights=None)
    assert isinstance(arch, torch.nn.Module)


def test_noninteger_output_nodes():
    with pytest.raises(TypeError):
        arch = cnn_architectures.resnet101(4.5, weights=None)
