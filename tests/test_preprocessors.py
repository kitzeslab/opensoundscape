...
def test_interrupt_get_item(dataset_df):
    ...
    dataset = SpectrogramPreprocessor(dataset_df, sample_duration=2.0)
...
def test_cnn_preprocessor_overlay(dataset_df, overlay_df):
    dataset = SpectrogramPreprocessor(
        dataset_df, sample_duration=2.0, overlay_df=overlay_df
    )
    sample1 = dataset[0]["X"]
...
def test_overlay_tries_different_sample(dataset_df, bad_good_df):
    dataset = SpectrogramPreprocessor(
        dataset_df, sample_duration=2.0, overlay_df=bad_good_df
    )
...
def test_return_labels_no_columns_warning(dataset_df):
    with warnings.catch_warnings(record=True) as w:
        ...
        SpectrogramPreprocessor(dataset_df[[]], sample_duration=2.0)
...
def test_cnn_preprocessor_fails_on_short_file(short_file_df):
    ...
    dataset = SpectrogramPreprocessor(short_file_df, sample_duration=2.0)
