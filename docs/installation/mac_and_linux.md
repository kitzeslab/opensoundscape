...
OpenSoundscape can be installed on Mac and Linux machines with Python >=3.10 using the pip command `pip install --upgrade opensoundscape`. We strongly recommend installing OpenSoundscape in a virtual environment to prevent dependency conflicts.
...
* Install opensoundscape using pip: `pip install --upgrade opensoundscape`
* Deactivate the environment when you're done using it: `conda deactivate`

### Intel-chip Macs (not Apple Silicon)

You may need to install pytorch from conda-forge since newer versions are not visible to pip. Run these lines in the terminal, replacing `NAME` with the desired name of your environment:
...
* Install OpenSoundscape in the environment: `pip install --upgrade opensoundscape`
