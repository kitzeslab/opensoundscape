        ...
        **kwargs,
        ...
        Args:
            ...
            power: Exponent for the magnitude spectrogram [default: 2]
                power=1 gives energy, power=2 gives power
                If you wish to recover the signal rms, use power=1 and dB_scale=False
                and librosa.feature.rms(S=spec.spectrogram, frame_length=window_samples)
            dB_scale: If True, rescales values to decibels, always representing power:
                x=10*log10(x) for power=2, x=20*log10(x) for power=1
        ...
        To recover a windowed signal RMS from a spectrogram, use power=1, window_fn=torch.ones, normalize_by_window_length=False
        and dB_scale=False when creating the spectrogram

        ```python
        spec = Spectrogram.from_audio(audio, power=1, window_fn=torch.ones, normalize_by_window_length=False, dB_scale=False)
        # produces a spectrogram equivalent to:
        # librosa.magphase(librosa.stft(audio.samples, window=np.ones, center=False,win_length=w,n_fft=w,hop_length=w//2))[0]

        rms = librosa.feature.rms(S=spec.spectrogram, frame_length=spec.window_samples)
        ```
        ...
        if normalize_by_window_length:
            spectrogram = spectrogram / (window_samples ** (power / 2))
        ...
        # convert to decibels
        if dB_scale:
            spectrogram = torch_to_dB(spectrogram, power=power)
...
class MelSpectrogram(Spectrogram):
    ...
    def from_audio(
        ...
        **kwargs,
        ...
        spec = super().from_audio(
            ...
            power=power,
            dB_scale=False,  # don't convert to dB scale yet
            fft_size=n_fft,
            **kwargs,
        )
        ...
        spec.spectrogram = mel_transform(torch.tensor(spec.spectrogram)).numpy()

        if dB_scale:
            spec.spectrogram = numpy_to_dB(spec.spectrogram, power=power)
    ...
    def plot(
        ...
        norm = matplotlib.colors.Normalize(vmin=range[0], vmax=range[1])
        freqs = self.frequencies / (1000 if kHz else 1)
        n_freqs, n_times = self.spectrogram.shape
