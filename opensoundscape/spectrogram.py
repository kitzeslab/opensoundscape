...
class Spectrogram:
    ...
    Attributes:
        ...
        frequencies: (np.ndarray) discrete frequency bins generated by FFT
        times: (np.ndarray) time from beginning of file to the center of each window
        ...
        audio_sample_rate: sample rate of audio from which spec was created
            [default: None]
        fft_size: number of FFT points used to create spectrogram [default: None]
    ...
    def pcen(self, **kwargs):
        """apply per-channel energy normalization (PCEN) to spectrogram
        ...
        Returns:
            np.ndarray: 2d array with PCEN applied to self.power_spectrogram
        """
        assert (
    ...
    def min_max_scale(self, feature_range=(0, 1)):
        ...
        the minimum and maximum values of the spectrogram
    ...
    def bandpass(self, min_f, max_f, out_of_bounds_ok=True):
        ...
        crops the 2-d array of the spectrogram to the desired frequency range
        by removing rows.
    ...
    def net_amplitude(self, signal_band, reject_bands=None):
        ...
        Args:
            ...
            reject_bands: list of [low, high] frequency ranges in Hz (negative contribution)

        Returns:
            np.ndarray: time-series array of net amplitude
        """
        ...
        **kwargs,
        ...
        Args:
            ...
            overlap_fraction: fractional temporal overlap between consecutive windows
                ...
                - Note: specify at most one of (hop_samples, overlap_fraction, overlap_samples)
            overlap_samples: number of overlapping samples between consecutive windows
                - Note: specify at most one of (hop_samples, overlap_fraction, overlap_samples)
            fft_size: number of fft points, see torchaudio.transforms.Spectrogram
                If None, defaults to window_samples
            n_mels: Number of mel bands to generate [default: 64]
            ...
            **kwargs: additional keyword arguments passed to torchaudio.transforms.Spectrogram
...
def plot_spectrograms(
    ...
    Args:
        ...
        frequency_range: optional (min_f, max_f) tuple to bandpass each spectrogram
            before plotting [default: None]
...
def torch_to_dB(spectrogram, power=2, eps=1e-10):
    """Convert a torch tensor spectrogram from linear scale to decibels (dB)

    Args:
        spectrogram: torch.Tensor containing linear-scale spectrogram values
        power: exponent of the spectrogram values; 1 for magnitude, 2 for power
            [default: 2]
        eps: small value added before log to avoid log(0) [default: 1e-10]

    Returns:
        torch.Tensor of dB-scaled spectrogram values
    """
...
def numpy_to_dB(spectrogram, power=2, eps=1e-10):
    """Convert a numpy array spectrogram from linear scale to decibels (dB)

    Args:
        spectrogram: np.ndarray containing linear-scale spectrogram values
        power: exponent of the spectrogram values; 1 for magnitude, 2 for power
            [default: 2]
        eps: small value added before log to avoid log(0) [default: 1e-10]

    Returns:
        np.ndarray of dB-scaled spectrogram values
    """
    assert power in (1, 2), "power must be 1 or 2 to convert to dB scale"
    factor = 20 if power == 1 else 10
