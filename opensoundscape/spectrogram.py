...
class Spectrogram:
    ...
    Attributes:
        frequencies: (list) discrete frequency bins generated by fft times: (list) time from
        beginning of file to the center of each window spectrogram: a 2d array containing
        10*log10(fft) for each time window decibel_limits: minimum and maximum decibel values in
        .spectrogram window_samples: number of samples per window when spec was created
            [default: none]
    ...
    def __init__(
        ...
        if not isinstance(decibel_limits, tuple):
            raise TypeError(
                f"Spectrogram.decibel_limits should be a tuple [length=2]. "
                f"Got {decibel_limits.__class__}"
            )
        ...
        if len(decibel_limits) != 2:
            raise TypeError(
                f"decibel_limits should be a tuple [length=2]. "
                f"Got {len(decibel_limits)}"
            )
    ...
    def from_audio(
        ...
        Args:
            ...
            decibel_limits: limit the dB values to (min,max)
    ...
    def from_audio(
        ...
        Args:
            ...
            overlap_fraction: fractional temporal overlap between consecutive windows
                ...
                - Note: cannot specify both overlap_samples and overlap_fraction
            fft_size: see scipy.signal.spectrogram's `nfft` parameter
            decibel_limits: limit the dB values to (min,max) (lower values set to min,
                higher values set to max)
            dB_scale: If True, rescales values to decibels, x=10*log10(x)
                - if dB_scale is False, decibel_limits is ignored
