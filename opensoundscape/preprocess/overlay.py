...
def overlay(
    ...
    Args:
        ...
        break_on_key: the name of a preprocessing step to stop at when
            preprocessing the overlayed samples (typically this action's name)
    ...
    while overlay_prob > np.random.uniform() and overlays_performed < max_overlay_num:
        try:
            ...
            if overlay_class is None:
                ...
                good_choice = False
                attempt_counter = 0
                max_attempts = 10  # if we try this many times, raise error
            ...
            overlay_sample = sample.preprocessor.forward(
                overlay_sample, break_on_key=break_on_key
            )
            ...
            if isinstance(sample.data, torch.Tensor):
            ...
            elif isinstance(sample.data, Audio):
                # ensure same duration by trimming or looping
                if overlay_sample.data.duration != sample.data.duration:
                    overlay_sample.data = overlay_sample.data.trim(
                ...
                raise PreprocessingError(
                    f"Overlay action cannot handle sample data of type {type(sample.data)}"
                )
