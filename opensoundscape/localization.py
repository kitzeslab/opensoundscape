...
class GetStartTimestamp:
    ...
    def __call__(self, index_tuple):
        ...
        # return self.tz.localize(recording_start_dt + datetime.timedelta(seconds=start_time))

        # if not (
        #     type(recording_start_dt) == datetime.datetime
        #     and recording_start_dt.tzinfo is not None
        # ):
        #     pass
    ...
    def create_candidate_events(
        ...
        Args:
            detections: a dictionary of detections, with multi-index (file,start_time,end_time,start_timestamp), and
                one column per class with 0/1 values for non-detection/detection
                where start_timestamp contains datetime.datetime corresponding to start_time
                If not present, attempts to automatically determine the start_timestamps file from the metadata's
                'recording_start_time' field.
        ...
        if not "start_timestamp" in detections.index.names:
            ...
            try:
                ...
                detections["start_timestamp"] = [
                    get_start_ts(i) for i in detections.index
                ]
