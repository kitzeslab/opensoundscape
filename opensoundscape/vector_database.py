...
def _insert_embeddings(db, batch_samples, batch_embeddings, overflow_mode, file_to_id, audio_root=None, deployment_id=None):
    ...
    batch_embeddings = batch_embeddings.clip(-max_float16, max_float16).astype(np.float16)
...
def _handle_existing_windows(db, clips, embedding_exists_mode):
    ...
    if embedding_exists_mode in ["skip", "error"]:
        ...
        id_to_recording = {rec.id: rec.filename for rec in db.get_all_recordings()}
        existing_index_tuples = {
            (id_to_recording[w.recording_id], w.offsets[0], w.offsets[1]) for w in windows
        }
